...
\begin{document}
	...
	We recall that for functions taking positive values,
	 ...
	 $$
	 l(\mu, \sigma^2) = \log \left( L(\mu, \sigma^2) \right) = \log \left( \prod_{i=1}^{n} f(x_i | \mu, \sigma^2) \right) = \log \left( \frac{n}{\sqrt{2 \pi \sigma^2}} \right) - \frac{1}{2\sigma^2} \sum_{i=1}^{n} (x-\mu)^2
	 $$
